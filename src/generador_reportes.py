...
class GeneradorReportes:
    ...
    def escribir_reporte(self, reporte_limpieza, particiones, matrices, topologias, comparaciones):
        ...
        md = f"""# Informe: Análisis de Comportamiento Académico por Bloques NCD/Gzip y Árboles Bayesianos
...
*   **Nivel 50%:** 
    * `Best_50`: Top 50% ({len(particiones.get('Best_50', {}).get('df', []))} estudiantes)
    * `Worst_50`: Bottom 50% ({len(particiones.get('Worst_50', {}).get('df', []))} estudiantes)
...
"""
        ...
        for nombre, info in matrices.items():
            ...
            nivel = info["nivel"]
            md += f"### Matriz NCD - {nombre} (Nivel {nivel}%)\n\n"
...
"""
        ...
        for nivel, df_comp in comparaciones.items():
            md += f"### Comparación Extrema para Nivel {nivel}%\n\n"
...
"""
        ...
        if "12.5" in comparaciones:
            ...
            md += f"1.  **Factores Determinantes:** Al comparar los bloques extremos (el 12.5% mejor vs el 12.5% peor), las variables cuyas relaciones y centralidad topológica cambian de forma más radical son **{max_cambio['Nombre']} ({max_cambio['Variable']})** y **{segundo_cambio['Nombre']} ({segundo_cambio['Variable']})**.\n"
            md += f"2.  **Mecánica de Impacto:** La variable `{max_cambio['Variable']}` pasa de tener un grado de {max_cambio['Grado_Best']:.4f} en los mejores estudiantes a {max_cambio['Grado_Worst']:.4f} en los peores ($D = {max_cambio['Diferencia_D']:+.4f}$).\n"
            md += f"3.  **Variables Invariantes:** Las variables que mostraron menor variación o que permanecieron casi idénticas en ambas redes son aquellas al final del ranking, lideradas por **{df_125.iloc[-1]['Nombre']} ({df_125.iloc[-1]['Variable']})**.\n"
